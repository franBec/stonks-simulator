...
INITIALIZE-STOCKS.
    ...
    MOVE 'YOLO Capital          ' TO WS-STK-Name(7)
...
PARSE-JSON.
    ...
    UNSTRING WS-INPUT-BUFFER
        ...
        INTO WS-TKN(1) WS-TKN(2) WS-TKN(3) WS-TKN(4)
             WS-TKN(5) WS-TKN(6) WS-TKN(7) WS-TKN(8)
             WS-TKN(9) WS-TKN(10) WS-TKN(11) WS-TKN(12)
        COUNT IN WS-TOKEN-COUNT

    PERFORM VARYING WS-I FROM 1 BY 1
        UNTIL WS-I > 11
        MOVE FUNCTION TRIM(WS-TKN(WS-I)) TO WS-FOUND-KEY
        IF WS-FOUND-KEY = 'action'
            ADD 1 TO WS-I
            MOVE FUNCTION TRIM(WS-TKN(WS-I))
                TO WS-REQ-ACTION
        ELSE IF WS-FOUND-KEY = 'symbol'
            ADD 1 TO WS-I
            MOVE FUNCTION TRIM(WS-TKN(WS-I))
                TO WS-REQ-SYMBOL
        ELSE IF WS-FOUND-KEY = 'quantity'
            ADD 1 TO WS-I
            MOVE FUNCTION NUMVAL(FUNCTION TRIM(WS-TKN(WS-I)))
                TO WS-REQ-QUANTITY
        ELSE IF WS-FOUND-KEY = 'price'
            ADD 1 TO WS-I
            MOVE FUNCTION NUMVAL(FUNCTION TRIM(WS-TKN(WS-I)))
                TO WS-REQ-PRICE
        ELSE IF WS-FOUND-KEY = 'cashBalance'
            ADD 1 TO WS-I
            MOVE FUNCTION NUMVAL(FUNCTION TRIM(WS-TKN(WS-I)))
                TO WS-REQ-CASH
        END-IF
...
VALIDATE-TRADE.
    ...
    IF WS-REQ-ACTION NOT = 'BUY' AND
       WS-REQ-ACTION NOT = 'SELL'
        ...
        MOVE WS-ERR-INVALID-ACTION TO WS-RES-ERROR-CODE
        MOVE 'JOB ABEND S225 - INVALID ACTION SPECIFIED'
            TO WS-RES-MESSAGE
    END-IF
    ...
    IF WS-RES-STATUS = 'ACCEPTED'
        ...
        PERFORM VARYING WS-STOCK-INDEX FROM 1 BY 1
            ...
            IF WS-REQ-SYMBOL = WS-STK-SYMBOL(WS-STOCK-INDEX)
                SET STOCK-FOUND TO TRUE
                MOVE WS-STOCK-COUNT TO WS-STOCK-INDEX
            END-IF
        END-PERFORM
        IF STOCK-NOT-FOUND
            MOVE 'REJECTED' TO WS-RES-STATUS
            MOVE WS-ERR-INVALID-SYMBOL TO WS-RES-ERROR-CODE
    ...
    IF WS-RES-STATUS = 'ACCEPTED'
        IF WS-REQ-QUANTITY = 0
            MOVE 'REJECTED' TO WS-RES-STATUS
            MOVE WS-ERR-INVALID-QTY TO WS-RES-ERROR-CODE
            MOVE 'JOB ABEND S224 - INVALID QUANTITY SPECIFIED'
                TO WS-RES-MESSAGE
        END-IF
    END-IF
    ...
    IF WS-RES-STATUS = 'ACCEPTED'
        IF WS-REQ-PRICE = 0
            MOVE 'REJECTED' TO WS-RES-STATUS
            MOVE WS-ERR-INVALID-PRICE TO WS-RES-ERROR-CODE
            MOVE 'JOB ABEND S226 - INVALID PRICE SPECIFIED'
                TO WS-RES-MESSAGE
        END-IF
    END-IF
    ...
    IF WS-RES-STATUS = 'ACCEPTED'
        IF WS-REQ-ACTION = 'BUY'
            IF WS-REMAINING-CASH < 0
                ...
                MOVE 'JOB ABEND S222 - INSUFFICIENT FUNDS'
                    TO WS-RES-MESSAGE
            END-IF
        END-IF
    END-IF.
...
BUILD-RESPONSE.
    ...
    IF WS-RES-STATUS = 'ACCEPTED'
        STRING
            '{"status":"ACCEPTED",' DELIMITED BY SIZE
            '"errorCode":null,' DELIMITED BY SIZE
            '"message":"TRADE VALIDATED - '
            'PROCEED TO EXECUTION",' DELIMITED BY SIZE
            '"totalCost":' DELIMITED BY SIZE
            FUNCTION TRIM(WS-DISPLAY-COST) DELIMITED BY SIZE
            ',"remainingCash":' DELIMITED BY SIZE
            FUNCTION TRIM(WS-DISPLAY-REMAINING) DELIMITED BY SIZE
            '}' DELIMITED BY SIZE
            INTO WS-JSON-RESPONSE
        END-STRING
    ELSE
        STRING
            '{"status":"REJECTED",' DELIMITED BY SIZE
            '"errorCode":"' DELIMITED BY SIZE
            FUNCTION TRIM(WS-RES-ERROR-CODE) DELIMITED BY SIZE
            '",' DELIMITED BY SIZE
            '"message":"' DELIMITED BY SIZE
            FUNCTION TRIM(WS-RES-MESSAGE) DELIMITED BY SIZE
            '",' DELIMITED BY SIZE
            '"totalCost":' DELIMITED BY SIZE
            FUNCTION TRIM(WS-DISPLAY-COST) DELIMITED BY SIZE
            ',"remainingCash":' DELIMITED BY SIZE
